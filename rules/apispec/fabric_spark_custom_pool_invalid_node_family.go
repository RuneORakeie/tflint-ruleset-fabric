...
func (r *FabricSparkCustomPoolInvalidNodeFamily) Severity() tflint.Severity { return tflint.ERROR }
...
func (r *FabricSparkCustomPoolInvalidNodeFamily) Check(runner tflint.Runner) error {
	...
	for _, block := range content.Blocks {
		...
		attr, ok := block.Body.Attributes["node_family"]
		if !ok {
			continue
		}
		...
		if false && len(v) > 0 {
			if err := runner.EmitIssue(r, fmt.Sprintf("%s exceeds max length %d", "node_family", 0), attr.Expr.Range()); err != nil {
				return err
			}
		}
		if false && len(v) < 0 {
			if err := runner.EmitIssue(r, fmt.Sprintf("%s shorter than min length %d", "node_family", 0), attr.Expr.Range()); err != nil {
				return err
			}
		}
