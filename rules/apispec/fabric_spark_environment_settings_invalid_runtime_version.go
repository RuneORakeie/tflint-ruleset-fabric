...
func (r *FabricSparkEnvironmentSettingsInvalidRuntimeVersion) Check(runner tflint.Runner) error {
	...
	for _, block := range content.Blocks {
		...
		if false && len(v) > 0 {
			if err := runner.EmitIssue(r, fmt.Sprintf("%s exceeds max length %d", "runtime_version", 0), attr.Expr.Range()); err != nil {
				return err
			}
		}
		if false && len(v) < 0 {
			if err := runner.EmitIssue(r, fmt.Sprintf("%s shorter than min length %d", "runtime_version", 0), attr.Expr.Range()); err != nil {
				return err
			}
		}
